...
def norm_img(img, percentile=99.50):
...
def patch_slicer(scan, mask, patch_size, stride, remove_bg=True, test=False, ori_path=None):
    ...
    if test:
        ...
        patch_idx = []
    if remove_bg:
        x_all = np.where(np.sum(np.sum(scan, axis=1), axis=1) > (0 * y * z))[0]
        y_all = np.where(np.sum(np.sum(scan, axis=0), axis=1) > (0 * x * z))[0]
        z_all = np.where(np.sum(np.sum(scan, axis=0), axis=0) > (0 * y * x))[0]
        x1 = x_all[0]
        x2 = x_all[-1]
        y1 = y_all[0]
        y2 = y_all[-1]
